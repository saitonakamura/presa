...
import {
  Presentation,
  Slide,
  Fragment as RegularFragment,
  FragmentProps,
  Presa,
} from '@saitonakamura/presa'
import { Code, H1, H3, VideoBackground } from '@saitonakamura/presa-blocks'
...
const Fragment = (props: FragmentProps) => (
  <RegularFragment {...props} behaviour={Hideable as any} />
)
...
const tick = ts => {
  ...
  redraw(delta)
    ...
    <Slide name="Getting started: code" centered>
      <Code language="javascript">{`import { Presentation, Slide } from 'presa'
...
ReactDOM.render(<App />, container)`}</Code>
    ...
    <Slide name="Slide backgrounds: code" centered>
      <Code language="javascript">{`// Use an image
...
<Slide background=
  "linear-gradient(to right, #da4453, #89216b)" />`}</Code>
    ...
    <Slide name="Video backgrounds: code" centered>
      <Code language="javascript">{`// blocks are optional elements
...
// (you can pass YouTube link or link to a local file)`}</Code>
    ...
    <Slide name="Code block" centered>
      <Code language="javascript">{`import { H1, H2, Code } from 'presa/blocks'
...
const NumberedCont = styled.div<{ centered?: boolean; inverse?: boolean }>`
  display: flex;
  flex-flow: column;
...
const ImageLayout = (children: React.ReactNode) => (
  <ImageCentred>{children}</ImageCentred>
)
...
const ImageBackground = styled(ImageCentred)<{ backgroundColor: string }>`
